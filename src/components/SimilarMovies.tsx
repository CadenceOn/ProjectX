    ...
    const fetchSimilarMovies = async () => {
      try {
        ...
        const transformedMovies = response.data.items.map((item: any) => ({
          ...
          genres: item.genres ? item.genres.map((g: any) => g.genre) : ["Неизвестно"],
        ...
        <div className="similar-movies-grid">
          {similarMovies.map((movie) => (
            <div
              ...
              onClick={() => navigate(`/project_x/movies/${movie.id}`)}
              role="button"
              tabIndex={0}
              onKeyPress={(e) => {
                if (e.key === "Enter") {
            ...
            >
              <div className="movie-content">
                ...
                <p className="movie-year"><strong>Год:</strong> {movie.year}</p>
